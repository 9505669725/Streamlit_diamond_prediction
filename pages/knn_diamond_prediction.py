...
st.title('Dimond price prediction app')
...
carat=st.number_input(label='carat',min_value=0.2,max_value=4.01)
cut=st.selectbox('cut',{'Fair','Good','Very Good','Premium','Ideal'})
color=st.selectbox('color',{'D','E','F','G','H','I','J'})
clarity=st.selectbox('clarity',{'I1','SI2','SI1','VS2','VS1','VVS2','VVS1','IF'})
r=pd.DataFrame()

regressor=st.sidebar.selectbox('selct algorithm',('LinearRegression','KNeighborsRegressor','RandomForestRegressor','GradientBoostingRegressor'))
...
test['carat']=scaler.fit_transform(test[['carat']])
...
if submit:
    ...
    st.write(f'the mae value is={mae}')
    st.write(f'the mse value is={mse}')

actual=st.checkbox('check the actual value')
if actual:
    st.write('the actual price is=$2616')